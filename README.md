...
**Server settings:** `MCP_AUTH_TOKEN` (bearer auth — set it in production), `MCP_API_KEYS` (scoped keys, see below), `MCP_BOOTSTRAP_ADMIN` (first-run: mint a random admin token instead of running unauthenticated, see below), `MCP_ALLOW_WRITES` (default `false`), `MCP_WRITE_DRYRUN` (default `false`), `MCP_HTTP_PORT` (8080), `MCP_RATE_LIMIT_PER_MINUTE` (300), `MCP_SESSION_IDLE_TIMEOUT_MINUTES` (30), `MCP_MAX_RESULT_CHARS` (50000), `MCP_TRUST_PROXY`, `LOG_LEVEL`.

**First-run bootstrap.** No credential is ever hardcoded. With nothing configured the server runs unauthenticated on loopback (and refuses to bind a network interface). For a quick secured start, set `MCP_BOOTSTRAP_ADMIN=true`: on boot, when no other auth is configured, it generates a random full-access admin token, prints it once to the logs, and persists it to `.mcp-bootstrap-admin` (mode `0600`, override with `MCP_BOOTSTRAP_ADMIN_FILE`) so it survives restarts. Use it as `Authorization: Bearer <token>`; set a real `MCP_AUTH_TOKEN` to take over.
...
**LDAP / Active Directory (native login)** — the self-service console can authenticate users directly against LDAP/AD with a username + password form (no IdP required). Set `AUTH_LDAP_URL` + `AUTH_LDAP_SEARCH_BASE` (and, for AD, a service account via `AUTH_LDAP_BIND_DN`/`AUTH_LDAP_BIND_PASSWORD`); the console does the standard search-then-bind, reads the user's `memberOf` groups, and grants write-capable key minting to members of `AUTH_LDAP_ADMIN_GROUPS`. Passwords are sent straight to the directory and never stored. All settings are exposed in the Helm chart's `values.yaml`. (Prefer SSO? Configure OIDC on the console instead — or bridge LDAP through Keycloak/Dex into OIDC. LDAP and OIDC can both be enabled; the login page offers a "Sign in with SSO" link.)
